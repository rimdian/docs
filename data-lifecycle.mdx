...
1. **Persist the raw data** in the `Database`.
2. **Enrich or reject the data** by calling `before_validation` hooks (IP-to-location, IP filtering...).
3. **Validate the data** against schemas (core + apps).
4. **Acquire a lock** on the data at the user-level, to ensure consistency.
5. **Reconciliate the user identity** to maintain a single view of the user.
6. **Reattribute conversions** to the right marketing campaigns for `order` events.
7. **Recompute user segments** to keep them up-to-date.
8. **Trigger workflows** who "subscribe" to the data point (e.g. sending an email when a new `order` is created).
9. **Forward the data** by calling the `on_suceess` hooks.
...
In the following example of a web `pageview` hit sent to the `Collector` by the JavaScript tracker, the `user`, `device` and `session` objects are attached to the `pageview`, to make sure we can heal the dataset if they went missing.
