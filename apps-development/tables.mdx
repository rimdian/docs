...
The `external_id` is the ID of the record in the external system, and the `id` is a SHA1 hash of the `external_id` used by Rimdian.

The `created_at` and `updated_at` columns are used to track the creation and update of the records in the external system. 
...
Relationships can be defined as `has_one`, `has_many`, or `belongs_to`.
